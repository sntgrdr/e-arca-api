...
RSpec.describe Invoices::Production::SendToArcaService, type: :service do
          ...
          <FECAESolicitarResponse xmlns="http://ar.gov.afip.dif.FEV1/">
            <FECAESolicitarResult>
              <FeCabResp>
                ...
                <FchProceso>20240101120000</FchProceso>
                <CantReg>1</CantReg>
              ...
              <FeDetResp>
                <FECAEDetResponse>
                  ...
                  <CbteFch>20240101</CbteFch>
                  <Resultado>A</Resultado>
                  <CAE>#{cae}</CAE>
                  <CAEFchVto>20240111</CAEFchVto>
                </FECAEDetResponse>
              </FeDetResp>
            </FECAESolicitarResult>
  ...
  def rejected_response_xml(error_msg: 'El contribuyente no puede emitir ese comprobante')
    <<~XML
      <?xml version="1.0" encoding="utf-8"?>
      <soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
        <soap:Body>
  ...
  describe '#call' do
    ...
    context 'when AFIP approves the invoice' do
      ...
      it 'sets the CAE on the invoice' do
        service.call
        expect(invoice.reload.cae).to eq('75109284616808')
      end
      ...
      it 'sets the CAE expiration date' do
        service.call
        expect(invoice.reload.cae_expiration).to eq(Date.parse('2024-01-11'))
      end

      it 'sets the afip_authorized_at timestamp' do
        service.call
      ...
      it 'sets afip_invoice_number' do
        service.call
        expect(invoice.reload.afip_invoice_number).to eq('1')
      end
    end

    context 'when AFIP rejects the invoice' do
      let(:error_message) { 'El contribuyente no puede emitir ese comprobante' }
    ...
    context 'when AFIP returns a malformed XML response (missing required nodes)' do
      before do
        stub_request(:post, afip_url)
          .to_return(status: 200, body: '<soap:Envelope><soap:Body></soap:Body></soap:Envelope>',
                     headers: { 'Content-Type' => 'text/xml' })
      end

      it 'raises a RuntimeError with the malformed response message' do
        expect { service.call }.to raise_error(RuntimeError, /Respuesta AFIP malformada/)
      end
    end
  end
end
