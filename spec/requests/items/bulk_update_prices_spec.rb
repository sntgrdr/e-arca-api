...
RSpec.describe "PATCH /api/v1/items/bulk_update_prices", type: :request do
  ...
  it "returns 422 when items list is empty" do
    ...
    expect(body.dig("error", "message")).to match(/No items/)
  end
  ...
  it "returns 422 with count when over 100 items" do
    ...
    body = JSON.parse(response.body)
    expect(body.dig("error", "code")).to eq("validation_error")
    expect(body.dig("error", "message")).to match(/received 101/)
  end
  ...
  it "returns 422 when all provided items belong to another user" do
    other_user = create(:user)
    other_item = create(:item, user: other_user, iva: iva, price: 100)
    patch "/api/v1/items/bulk_update_prices",
          params: { items: [ { id: other_item.id, price: 121.0 } ] },
          headers: headers, as: :json
    expect(response).to have_http_status(:unprocessable_entity)
    body = JSON.parse(response.body)
    expect(body.dig("error", "code")).to eq("validation_error")
    expect(body.dig("error", "message")).to match(/No valid items/)
  end

  it "returns 422 when all provided items are inactive" do
    inactive_item = create(:item, user: user, iva: iva, price: 100, active: false)
    patch "/api/v1/items/bulk_update_prices",
          params: { items: [ { id: inactive_item.id, price: 121.0 } ] },
          headers: headers, as: :json
    expect(response).to have_http_status(:unprocessable_entity)
    body = JSON.parse(response.body)
    expect(body.dig("error", "code")).to eq("validation_error")
  ...
  it "returns 422 when the items param is missing entirely" do
    patch "/api/v1/items/bulk_update_prices",
  ...
  it "returns the correct price_with_iva in the response" do
    patch "/api/v1/items/bulk_update_prices",
          params: { items: [ { id: item1.id, price: 121.0 } ] },
          headers: headers, as: :json
    expect(response).to have_http_status(:ok)
    item_json = JSON.parse(response.body).first
    expect(item_json["price_with_iva"].to_f).to eq(121.0)
  end
